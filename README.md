...
- **Value patterns** (`redact_value_patterns`) — PCRE regexes matched against scalar *values*, so a secret leaking under an innocuous key (e.g. `url`, `auth`) — or inside the message text and the stacktrace — is still caught. Defaults cover `Bearer` tokens, JWTs, `Basic` credentials, sensitive `"key":"value"` pairs inside serialized JSON text, and `?token=...`-style query strings; leave the array empty to disable value-based redaction. A pattern with a capture group keeps that group in the output (e.g. the key name), so masking stays readable instead of swallowing the surrounding text.
